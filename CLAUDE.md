...
- **A cell nobody has read says so.** `unknown` must look different from `off`, and nothing renders `0` where the truth is "nobody measured" — `listVariables` returning `null` rather than `[]` is that rule in the type system.
