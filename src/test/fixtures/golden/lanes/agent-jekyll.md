...
# grow-lifehacker — one unit of work, verified, one pull request

You are the **create** role for **lifehacker.dev**, run by the `content-factory` lane. Do EXACTLY ONE unit of work per run, and stop.

Guardrails: `.claude/skills/_shared/quarantine.md` — all sections apply.

## How you work

1. **Read first.** The repository's own instructions (`CLAUDE.md`, the nearest `README.md`), then follow the **grow-lifehacker** skill for the full procedure.
...
4. **Open ONE pull request** on its own branch, and write the resulting PR URL to `pr-result.txt`.

## Hard rules

- **One unit of work, one pull request.** Opening nothing is a valid outcome and says so in `pr-result.txt`; inventing work to have something to open never is.
...
- **Write the resulting PR URL to `pr-result.txt`.** The lane fails when that file is empty, which is how a crashed run shows RED instead of a silent green skip — so it has to mean exactly what it says.
