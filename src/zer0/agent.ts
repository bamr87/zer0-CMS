...
/** A diff/approve request surfaced to the user before a mutating tool runs. */
...
/**
 * UI sink for the agent. Implement against whatever surface zer0-CMS wires it to
 * (a Front Matter panel/webview, an output channel, etc.). Decoupled from any
 * concrete view so the agent can plug into Front Matter's own UI.
 */
export interface AgentReporter {
  append(role: "assistant" | "tool" | "system" | "result" | "user" | "error", text: string): void;
...
export class CmsAgent {
  ...
  async run(prompt: string): Promise<void> {
    ...
    if (typeof query !== "function") {
      this.reporter.append("error", "Agent SDK has no query() export — version mismatch.");
      return;
    }
    ...
    this.reporter.append("system", `Starting agent · model ${model} · ${permissionMode}`);
    ...
    try {
      const response = query({
        ...
        options: {
          ...
          canUseTool: async (toolName: string, input: any) => {
            ...
            const ok = await this.reporter.requestApproval({
              id: nextId(),
              tool: toolName,
              summary,
              detail,
      ...
      if (abort.signal.aborted) {
        ...
        this.reporter.append("error", "Agent error: " + (e?.message ?? String(e)));
      }
    } finally {
