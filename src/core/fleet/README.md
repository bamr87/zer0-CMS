...
**`listVariables()` returns `null`, not `[]`, on 403/404, and that tristate is load-bearing.** An empty list means the repository really has no variables, so every switch is genuinely `unset` and every lane is genuinely off. `null` means nobody could ask. A console that rendered the second as the first would draw a running lane as a stopped one, which is the single most expensive lie this surface could tell. `listWorkflows()`, `recentRuns()` and `openPulls()` keep the same tristate: `null` on 403/404, `[]` only for a 200 that listed nothing. The shell carries it as `runsReadable`/`workflowsReadable` beside `switchesReadable`, the run-verb gates refuse an unread list with "could not be read" rather than "has no failed run" or "has no registered workflow", and `engineClientOver` throws a 403 `FleetGithubError` instead of handing the engines a fabricated `[]`.
