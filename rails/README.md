...
| LinkedIn | `/admin/distribution`, `/admin/channels`, `app/services/linkedin_gateway.rb` | Per site: the queue, each draft's exact request and gates, **Approve**, **Publish** (armed only by `ZER0_LINKEDIN_PUBLISH=1` on the server), statistics. A channel connects an account through LinkedIn's consent screen and keeps its token encrypted. Every action runs `Zer0Cms::Distribution::Pipeline`, the code `bin/zer0-cms linkedin` runs ([`docs/DISTRIBUTION.md`](../docs/DISTRIBUTION.md)) |
