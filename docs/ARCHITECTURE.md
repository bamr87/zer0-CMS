...
This repository is two products that share a git remote: a **VS Code extension** (`src/`) and a **Rails fleet CMS** (`rails/`). This document is the extension's internal boundary. The browser platform's contract is [`PLATFORM.md`](PLATFORM.md); the pipeline is [`CICD.md`](CICD.md).

The extension has a hard internal boundary. This section explains where that boundary is, why it is there, and what it buys.

## Design decisions D1–D16

The first fourteen decisions are the reason the extension's code looks the way it does; D15 and D16 govern the Rails half and its boundary with the extension. They were made before the first line was written and lived, until now, only in a design conversation — fifteen source and doc files cited them as `PLAN §x.y` against a document that was never in this repository. They are restated here from the code, so a citation resolves and an amendment has somewhere to land. Each names the mechanism that enforces it, because a decision no mechanism defends is a preference.
...
| **D10** | **`canUseTool` is the single agent gate.** `allowedTools` is never passed to the SDK, because a tool allow-list negotiated at session start cannot show a person the diff it is about to write. Read-only tools run freely; everything else goes through an approve/deny card. **`acceptEdits` is not an available mode** — measured against the SDK, a `Write` under it landed on disk with `canUseTool` never called, so a setting this extension itself offered was quietly disarming the gate. It is gone from the manifest and clamped at every layer. | `QueryOptions` deliberately omits `allowedTools`; `AgentPermissionMode` is two values; the file and settings layers both clamp; `READ_ONLY_TOOLS` |
...
| **D15** | **The browser CMS and the extension share on-disk content, not a process.** Rails (`rails/`) is the fleet CMS; the extension stays the in-editor editor, and neither is folded into the other. Both edit front matter by line surgery — the extension with `updateFrontMatterKeys`, Rails with its port `Zer0Cms::Cms::FrontMatter.update_keys` — so a save from either leaves every untouched line byte-identical (D7). The library under `rails/lib` stays stdlib-only; gems belong in the Rails app. *Amended 2026-09-14:* the Ruby port is held to the extension's cases (fence kept, insertion before the closing fence, block continuation lines, YAML-correct quoting, deletion, CRLF and BOM), and the Rails catalog reads a site with Jekyll 4.4's own reader rules rather than a glob, so both surfaces agree with Jekyll about what is content. | `rails/bin/test-stdlib` in `abc-engine.yml` (no bundler, Ruby 3.3 and 4.0.5); `test_front_matter.rb`; `test_catalog.rb` against expected lists Jekyll 4.4.1 generated (`rails/bin/jekyll-parity`); [`PLATFORM.md`](PLATFORM.md) |
| **D16** | **Administrate over a disk-synced index; git stays the source of truth.** The Rails UI is Administrate 1.0 dashboards over four ActiveRecord models — `Site`, `Page`, `Asset`, `Term` — that are only an index: `Site#sync!` rebuilds a site's rows from disk in one transaction, upserting by natural key and deleting what vanished. Nothing is written through ActiveRecord. Every write goes to the file: `PageEditor` re-reads it, refuses when its digest differs from the index, edits by line surgery, renames atomically and re-syncs that one path. Administrate supplies what is general — index search, sort and pagination, generated show and edit forms, collection filters, custom fields and actions, namespaced routes — and zer0-CMS writes only what is specific: the sync, the write-back editor, the image-engine bridge and the ABC wizard. Chosen 2026-09-14 on evidence: administrate 1.0.0, activeadmin 4.0.0.beta22, avo 4.2.3, trestle 0.10.1, alchemy_cms 8.3.9 and spina 2.21.0 all resolve with rails 8.1.3, propshaft, importmap-rails, turbo, stimulus and sqlite3. Administrate is MIT, stable 1.0 since 2025-10-31, with overridable ERB views and no asset build. activeadmin is still a beta and needs a Tailwind build; avo is LGPL-3.0 plus a commercial tier that gates the useful features; trestle is LGPL-3.0 with no release in twelve months; alchemy_cms and spina keep pages in the database, so content would fork away from Jekyll and git would stop being the source of truth. | `PageEditor` and `Zer0Cms::Cms::Writer` are the only writers; `page_editing_test.rb` (an unchanged save of every indexed file is byte-for-byte a no-op; a stale file or form is refused); `site_sync_test.rb`; `rails-app.yml` |
| **D17** | **Distribution is one stdlib lane over a site's own files, with a declared call plan; files describe, the environment arms.** LinkedIn publishing lives in `rails/lib/zer0_cms/linkedin` and `rails/lib/zer0_cms/distribution`, so the CLI, the reusable `zer0-linkedin.yml` workflow, the MCP server and the Rails CMS all run one `Pipeline` — one gate to audit. Every LinkedIn call is a `Plan::Call` with a sentence saying what comes back, and `Client#request` takes a call id, never a URL. The queue, the brand guard, the ledger and `.cms/distribution/performance.json` keep the extension's dialect, rule names and byte format, so a draft, a finding or a statistic means the same thing in the editor, the browser and CI, and the extension's catering worklist ranks subjects from numbers this lane fetched. A live publish needs `ZER0_LINKEDIN_PUBLISH=1` in the environment of the process doing it; no `zer0.json` key can arm it (D13's rule for a cloned repository). The one secret the Rails app stores is a channel's OAuth token, encrypted with keys derived from `secret_key_base`; everything about content stays on disk (D16). Chosen 2026-09-15 to replace bash-365.com's per-site Python publisher and the zer0-distribute prototype; [`DISTRIBUTION.md`](DISTRIBUTION.md) is the contract. | `test_linkedin.rb` (every read returns own content; no declared path is forbidden; an undeclared call, a foreign upload host and an unheld scope are refused before a socket; a write is not retried after a 5xx); `test_distribution.rb` (the gate order; a preview writes and sends nothing; the ledger bytes equal Python's `json.dump`; a file cannot arm; force never lifts approval or arming); `distribution_test.rb` (an OAuth `state` is single-use; a stored token is encrypted and never rendered) |
...
**Which site a command acts on is decided by the command, not by the window.** A command invoked on a file acts on that file's site — the file names its folder, and the folder names its configuration. Only a command with no argument falls back to the *active* site, and that is resolved in this order:

1. the site a person explicitly picked, if it still exists,
2. the folder owning the active editor,
3. the first folder.

The rule lives in a pure function with tests, because a rule like this quietly acquires a fourth case, and one nobody can test is one nobody can change.

**Settings follow the folder.** Most of the 44 settings are `resource`-scoped, so each folder answers them for itself: that is what lets one window hold a Jekyll site and an MkDocs site without either one's conventions leaking into the other. The gates keep their own discipline — `governance.publishAllow` and the two fleet switches are read from the settings layer alone, per folder, so arming one site arms one site. Workspace Trust remains the outer gate and is per *workspace*, not per folder: a window is all-trusted or all-restricted, and in a restricted one nothing runs anywhere in it.

**The MCP server is per site.** One definition per configured, trusted folder, each rooted in its own directory, because a server that had to be told which site to answer about would be a server that could be told the wrong one. The two-phase registration is unchanged: the definitions carry an empty environment, and the flag is read at resolve time, from that folder's settings, and nowhere else.
...
**Two guards keep that list honest, and they were both tightened when it grew.** `fleetSurfaceIsRepoScopedOnly` used to check only that a path began with `/repos/{owner}/{repo}` — a prefix test that would have admitted `/issues`, `/hooks`, `/keys`, `/collaborators` and `/actions/secrets` the moment anyone added one. It now matches an explicit allow-list of sub-roots, whole-segment. `fleetPlanHasNoMergeVerbs` is the second: it refuses `/merge`, `/reviews`, `/update-branch`, `/actions/secrets`, `/collaborators`, any `DELETE`, and any write to `/labels`. Both are asserted from both directions — every planned call passes, and a hand-written forbidden one fails. A `..` segment is refused before a URL is built, because `fetch` would have normalised it into an endpoint the plan never named.
...
**Slice 2 is a fleet.** A roster — the workspace's own folders that carry a manifest, plus repositories you enrolled in your settings, plus, only when you ask, the hub's registry — and per repository four calls per Refresh rather than four per lane. Beside each lane: its switch, its newest run, the open pull requests attributed to it, what it has cost, and the grade the shared audit gives its workflows. Three more verbs act on a run rather than on configuration: re-run the last failure, cancel what is in flight, disable the workflow file.
...
**One vocabulary, two projections.** The agent in this editor and the AI lanes in CI used to share nothing: the editor did not read the repository's own model configuration, did not know its `.claude/agents` roles, did not attach this extension's MCP server, and defaulted to a model the fleet does not use — so "run this as the content role" was possible in a workflow and impossible at the desk, and the two disagreed about the model without anyone noticing. A harness profile now resolves the repository's roles, skills and model once, in the runner's own precedence (a setting, then `zer0.json`, then the site's AI configuration, then the built-in default), and projects to either an editor run or the equivalent CI invocation. The console can hand you that invocation to paste into a workflow.

The differentiator between the two is the human gate, not a second runner. Which is why the `settingSources` question matters: loading a repository's `.claude/settings.json` means its hooks — command lines, under your credential — apply to a run you started. That is off unless the workspace is trusted *and* you opt in for that run, and the extension's own MCP server is attached with the publish and scaffold flags explicitly removed from the child environment, because an editor run must not inherit whatever armed something else.
