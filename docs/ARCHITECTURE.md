...
| **D15** | **The browser CMS and the extension share on-disk markdown, not a process.** Rails (`rails/`) is the fleet panel; the extension stays the in-editor editor. Front-matter surgery in `lib/zer0_cms/cms/` must stay stdlib-only and comment-preserving, matching D7. Gems belong in the Rails host, never in `lib/`. | `rails/bin/test-stdlib` in `abc-engine.yml` (no bundler); [`PLATFORM.md`](PLATFORM.md) |
